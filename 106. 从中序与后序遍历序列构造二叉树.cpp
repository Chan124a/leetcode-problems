...
public:
    TreeNode* build(vector<int>& inorder, vector<int>& postorder, int inL, int inR, int posL, int posR) {
        ...
        int k = 0;
        while (inorder[inL + k] != postorder[posR])k++;
        root->left = build(inorder, postorder, inL, inL + k - 1, posL, posL + k-1);
        root->right = build(inorder, postorder, inL+k+1, inR, posL+k, posR-1);
        return root;
